...
function DraggableButton({
    ...
    const handleMove = useCallback(
        (event) => {
            ...
            if (!dataRef.current.isTouchEvent && !dataRef.current.isMouseDown)
                return;
            const clientX = dataRef.current.isTouchEvent
                ? event.changedTouches[0].clientX
                : event.clientX;
            const clientY = dataRef.current.isTouchEvent
                ? event.changedTouches[0].clientY
                : event.clientY;

            const { left, right, top, bottom } =
                closeButtonElemRef.current.getBoundingClientRect();
            ...
            if (closeButtonHovered) {
                closeButtonElemRef.current.style.transform =
        ...
        (event) => {
            ...
            if (isDragging) {
                ...
                if (windowHeight - clientY < verticalThreshold) {
                    yPosition =
                        windowHeight - (verticalThreshold + clientHeight / 2);
                } else if (
                    windowHeight - clientY >
                    windowHeight - verticalThreshold
                ) {
    ...
    const wrapperClass =
        `container ${className} ${isVisible ? '' : 'display-none'}`.trim();
